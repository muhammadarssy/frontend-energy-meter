...
const ProductCategories = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => getProductCategories({ page, limit })
  })
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['product-categories'] }); setDeleteTarget(null) }
  })
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    {['No', 'Kode', 'Nama', 'Deskripsi', 'Digunakan', 'Status', 'Aksi'].map((h) => (
                  ...
                  {items.length === 0 ? (
                    ...
                    <tr key={item.id} className={idx % 2 === 1 ? 'bg-gray-50' : 'bg-white'}>
                      ...
                      <td className="px-4 py-3 text-gray-600">
                        {item._count?.mappings ?? item.mappings?.length ?? '—'} produk
                      </td>
      ...
      <Modal
        isOpen={modalOpen}
        onClose={closeModal}
        title={editItem ? 'Edit Kategori' : 'Tambah Kategori Produk'}
